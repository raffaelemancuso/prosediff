...
- **It compares Word documents directly**: `prosediff --files draft.docx
  ...
  OpenDocument texts (.odt) from LibreOffice, on the Markdown that `pandoc`
  makes of a Word file, and on Word files in git, where `prosediff
...
| `-o`, `--output FILE`      | output file (default `diff.html`; with `--open`, a new page in the temporary folder)                             |
...
| `--to-markdown FILE`       | print a Word or OpenDocument file as the Markdown prosediff compares, tracked changes as `--docx-changes` says: what `git diff` shows once set up |
...
- `git diff`, `git log -p` and `git show` show a Word or OpenDocument file
  as the Markdown prosediff reads it into, instead of "Binary files
...
package and resolves the styles that make a span bold or italic, into the
same Markdown: headings, lists, tables, footnotes, links, and each comment
(`office:annotation`) where it starts. Its tracked changes are settled as
